...
function PurchaseHistoryList() {
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (status) => (
        <Tag
          color={status === "Success" ? "#389e0d" : "#ff4d4f"}
  ...
  if (error)
    return <Empty resourceName="lịch sử mua điểm" message={error.message} />;
  if (!purchaseHistories?.data?.length)
    return <Empty resourceName="lịch sử mua điểm" />;
