...
import { useHistoriesByStudentId } from "./useHistoriesByStudentId";
...
function HistoriesByStudentId() {
  const [limit, setLimit] = useState();
  const [sortField, setSortField] = useState("Id");
  const [sortOrder, setSortOrder] = useState("desc");
  const { historiesByStudentId, isLoading: ordersLoading } =
    useHistoriesByStudentId(limit, sortField, sortOrder);
  ...
  if (!historiesByStudentId?.result?.length)
      ...
      <Container>
        ...
        <Menus>
          <Table columns="0.4fr 3.2fr 1fr 1.1fr 1fr 1fr">
            ...
            <Table.Body
              data={historiesByStudentId?.result}
              render={(history, index) => (
                <HistoryRow
                  ...
                  index={index + 1}
                  displayedIndex={
                    (historiesByStudentId.currentPage - 1) *
                      historiesByStudentId.pageSize +
                    index +
                    1
                  }
                />
            ...
            <Table.Footer>
              <Pagination
                count={historiesByStudentId?.rowCount}
                currentPage={currentPage}
                pageSize={historiesByStudentId?.pageSize}
                pageCount={historiesByStudentId?.pageCount}
                totalCount={historiesByStudentId?.totalCount}
              />
              <SetRowsPerPage
                pageSize={historiesByStudentId?.pageSize}
...
export default HistoriesByStudentId;
