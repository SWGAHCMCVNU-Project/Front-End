...
function CreateCampaignVoucher({ selectVoucher, cost, disabled }) {
  ...
  // Khởi tạo selectedVouchers từ newCampaign.campaignDetails
  useEffect(() => {
    if (!hasInitialized) return;

    if (newCampaign?.campaignDetails) {
      // Nếu campaignDetails là string (từ trước đó), parse thành array
      if (typeof newCampaign.campaignDetails === "string" && newCampaign.campaignDetails !== "") {
        try {
          const parsedDetails = JSON.parse(`[${newCampaign.campaignDetails}]`);
          const selectedVoucherIds = parsedDetails.map((option) => option.voucherId) || [];
      ...
      // Nếu campaignDetails là array (định dạng đúng), sử dụng trực tiếp
      else if (Array.isArray(newCampaign.campaignDetails) && newCampaign.campaignDetails.length > 0) {
    ...
    if (selectedVouchers.length > 0) {
      ...
      setNewCampaign((prev) => {
        const updatedCampaign = {
          ...prev,
          campaignDetails: selectedVouchers, // Lưu dưới dạng array
        };
      ...
      if (updatedOptions[optionIndex].numberOfItems >= editingQuantity) {
        ...
        const updatedVouchers = selectedVouchers.map((voucher) => {
          if (voucher.voucherId === updatedOptions[optionIndex].value) {
            return { ...voucher, quantity: editingQuantity || 1, description: editingDescription || voucher.description };
          }
          return voucher;
        });
  ...
  const emptyRow = {
    ...
    number: <div className="number-header"><span>-</span></div>,
  ...
  const handleSelectVoucher = (value) => {
    ...
    const updatedItems = value.map((option) => {
      const existingVoucher = selectedVouchers.find((v) => v.voucherId === option);
      return existingVoucher || {
        voucherId: option,
        quantity: 1,
        fromIndex: 1,
        description: campaignVoucherOptions.find((o) => o.value === option)?.label || '',
        state: true
      };
    });
