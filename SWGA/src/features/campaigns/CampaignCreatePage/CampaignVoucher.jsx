...
function CampaignVoucher({
  ...
  useEffect(() => {
    ...
    if (walletBalance !== null && newTotal > walletBalance) {
      toast.error(
        `Số dư ví không đủ! Cần ${newTotal} điểm, nhưng ví chỉ có ${walletBalance} điểm.`
      );
    }
  ...
  const handleSave = (record) => {
    ...
    if (isNaN(editingQuantity) || editingQuantity < 1 || editingQuantity > 100) {
      toast.error("Số lượng không hợp lệ (1-100)");
      return;
    }
            ...
            return isEditable ? (
              <Input
                ...
                onChange={(e) => {
                  ...
                  setEditingQuantity(value === "" ? 0 : parseInt(value, 10));
                }}
                min="1"
                max="100"
