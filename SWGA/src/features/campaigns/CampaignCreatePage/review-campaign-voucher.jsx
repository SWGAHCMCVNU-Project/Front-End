...
function ReviewCampaignVoucher() {
    ...
    useEffect(() => {
        ...
        // Step 1: Fetch and filter vouchers
        const getVoucherSelectBox = () => {
            ...
            // Include all vouchers that are either active or were previously selected
        ...
        // Step 2: Update selectedItems after campaignVoucherOptions is set
        if (newCampaign?.campaignDetails !== undefined) {
            const selectedVoucherIds = newCampaign?.campaignDetails?.map((option) => option.voucherId) || [];
            console.log("Selected voucher IDs from newCampaign:", selectedVoucherIds);
    ...
    const data = selectedItems.map((value, index) => {
        ...
        const campaignDetail = newCampaign?.campaignDetails?.find((v) => v.voucherId === value);
