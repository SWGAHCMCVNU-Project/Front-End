...
function TodayActivity() {
  const { brandRankingAdmin, isLoading } = useBrandRanking();

  return (
    <StyledToday>
      <StyledHeading>
        <Heading as="h2">Bảng xếp hạng thương hiệu</Heading>
        <Heading as="h3">Số lượng: {brandRankingAdmin?.length}</Heading>
      </StyledHeading>

      {!isLoading ? (
        brandRankingAdmin?.length > 0 ? (
          <DataList>
            {brandRankingAdmin.map((activity, index) => (
