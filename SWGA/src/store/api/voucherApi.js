...
export const getVoucherByIdAPI = async (id) => {
  try {
    ...
    return {
      status: error.response?.status || 500,
      data: {
        message: error.response?.data?.message || "Lấy chi tiết phiếu ưu đãi thất bại",
      },
    };
  }
};
...
export const updateVoucherAPI = async (id, data) => {
  try {
    ...
    const endpoint = VOUCHER_ENDPOINTS.UPDATE.replace("{id}", id);
    ...
    // toast.success("Cập nhật phiếu ưu đãi thành công!");
    ...
    if (error.response?.data) {
      toast.error(error.message || "Cập nhật phiếu ưu đãi thất bại");
    }

    throw {
      status: error.response?.status || 500,
      message: error.message || error.response?.data?.message || "Cập nhật phiếu ưu đãi thất bại",
      details: error.response?.data
    };
  }
};